...
def main():
    ...
    n_ds = xr.load_dataset(
        os.path.join(
            directory, "peaks_indexes_tp_North_2021-11-04_08:06:42_ipi3_th_4.nc"
        )
    )
    s_ds = xr.load_dataset(
        os.path.join(
            directory, "peaks_indexes_tp_South_2021-11-04_08:06:42_ipi3_th_5.nc"
        )
    )

    n_ds_gab = xr.load_dataset(
        os.path.join(
            directory_gab, "peaks_indexes_tp_North_2021-11-04_08:06:42_ipi3_th_4.nc"
        )
    )
    s_ds_gab = xr.load_dataset(
        os.path.join(
            directory_gab, "peaks_indexes_tp_South_2021-11-04_08:06:42_ipi3_th_5.nc"
        )
    )
    ...
    # Load associations
    baseline_assoc = dw.assoc.load_assoc(
        "data/associations/Batch4/Baseline/association_2021-11-04_08:06:42.h5"
    )
    fw_assoc = dw.assoc.load_assoc(
        "data/associations/Batch4/FarWin/association_2021-11-04_08:06:42.h5"
    )
    gabor_assoc = dw.assoc.load_assoc(
        "data/associations/Batch4/Gabor_Farwin/association_2021-11-04_08:06:42.h5"
    )

    nhf_assoc_list_pair = baseline_assoc["assoc_pair"]["north"][
        "hf"
    ]  # List to store paired associated picks for the North cable, HF calls
    nlf_assoc_list_pair = baseline_assoc["assoc_pair"]["north"][
        "lf"
    ]  # List to store paired associated picks for the North cable, LF calls
    nhf_assoc_list = baseline_assoc["assoc"]["north"][
        "hf"
    ]  # List to store associated picks for the North cable, HF calls
    nlf_assoc_list = baseline_assoc["assoc"]["north"][
        "lf"
    ]  # List to store associated picks for the North cable, LF calls

    shf_assoc_list_pair = baseline_assoc["assoc_pair"]["south"][
        "hf"
    ]  # List to store paired associated picks for the South cable, HF calls
    slf_assoc_list_pair = baseline_assoc["assoc_pair"]["south"][
        "lf"
    ]  # List to store paired associated picks for the South cable, LF calls
    shf_assoc_list = baseline_assoc["assoc"]["south"][
        "hf"
    ]  # List to store associated picks for the South cable, HF calls
    slf_assoc_list = baseline_assoc["assoc"]["south"][
        "lf"
    ]  # List to store associated picks for the South cable, LF calls
    ...
    n_annot = pd.read_csv(
        f"{annot_root}/Batch{batch}/annotated_calls_north_{timestamp}.csv"
    )
    s_annot = pd.read_csv(
        f"{annot_root}/Batch{batch}/annotated_calls_south_{timestamp}.csv"
    )
    ...
    legend_container = inset_axes(
        ...
        bbox_transform=ax_south.transAxes,
    )
    ...
    legend_container.axvline(
        x=separator_x, ymin=0.2, ymax=0.8, color="lightgray", linewidth=1, alpha=0.7
    )

    plt.savefig("figs/Figure6bis_a.pdf", bbox_inches=None, transparent=True)
    # plt.show()
    ...
    nhf_assoc_list_pair = fw_assoc["assoc_pair"]["north"][
        "hf"
    ]  # List to store paired associated picks for the North cable, HF calls
    nlf_assoc_list_pair = fw_assoc["assoc_pair"]["north"][
        "lf"
    ]  # List to store paired associated picks for the North cable, LF calls
    nhf_assoc_list = fw_assoc["assoc"]["north"][
        "hf"
    ]  # List to store associated picks for the North cable, HF calls
    nlf_assoc_list = fw_assoc["assoc"]["north"][
        "lf"
    ]  # List to store associated picks for the North cable, LF calls

    shf_assoc_list_pair = fw_assoc["assoc_pair"]["south"][
        "hf"
    ]  # List to store paired associated picks for the South cable, HF calls
    slf_assoc_list_pair = fw_assoc["assoc_pair"]["south"][
        "lf"
    ]  # List to store paired associated picks for the South cable, LF calls
    shf_assoc_list = fw_assoc["assoc"]["south"][
        "hf"
    ]  # List to store associated picks for the South cable, HF calls
    slf_assoc_list = fw_assoc["assoc"]["south"][
        "lf"
    ]  # List to store associated picks for the South cable, LF calls
    ...
    # +
    nhf_assoc_list_pair = gabor_assoc["assoc_pair"]["north"][
        "hf"
    ]  # List to store paired associated picks for the North cable, HF calls
    nlf_assoc_list_pair = gabor_assoc["assoc_pair"]["north"][
        "lf"
    ]  # List to store paired associated picks for the North cable, LF calls
    nhf_assoc_list = gabor_assoc["assoc"]["north"][
        "hf"
    ]  # List to store associated picks for the North cable, HF calls
    nlf_assoc_list = gabor_assoc["assoc"]["north"][
        "lf"
    ]  # List to store associated picks for the North cable, LF calls

    shf_assoc_list_pair = gabor_assoc["assoc_pair"]["south"][
        "hf"
    ]  # List to store paired associated picks for the South cable, HF calls
    slf_assoc_list_pair = gabor_assoc["assoc_pair"]["south"][
        "lf"
    ]  # List to store paired associated picks for the South cable, LF calls
    shf_assoc_list = gabor_assoc["assoc"]["south"][
        "hf"
    ]  # List to store associated picks for the South cable, HF calls
    slf_assoc_list = gabor_assoc["assoc"]["south"][
        "lf"
    ]  # List to store associated picks for the South cable, LF calls

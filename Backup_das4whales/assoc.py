...
def process_iteration(
    ...
    if np.max(hf_kde) < mu_hf + 1.5 * sigma_hf and np.max(lf_kde) < mu_lf + 1.5 * sigma_lf:  # 3sig for not Gabor, 1sig for Gabor
        print("Stop criterion reached, ending association loop.")
    ...
    # Narrow down the selection window
    # dt_sel_narrowed = .7 # seconds Bet settings for not Gabor
    dt_sel_narrowed =.7 # seconds Best settings for Gabor
    ...
    if hf_north_south_good and lf_north_south_good:
        ...
        if max_time_hf >= -1: # Do not associate the edge cases
        ...
        # Then process LF
        if max_time_lf >= -1: # Do not associate the edge cases
    ...
    # First priority: Case 1 - HF North and South are good
        ...
        # Remove the LF hyperbola from the grid array to avoid reusing it
        ...
        n_arr_tg = np.delete(n_arr_tg, lf_imax, axis=0)
        s_arr_tg = np.delete(s_arr_tg, lf_imax, axis=0)
        n_shape_x = n_arr_tg.shape[0]
        s_shape_x = s_arr_tg.shape[0]
    ...
    # Second priority: Case 2 - LF North and South are good  
        ...
        # Remove the HF hyperbola from the grid array to avoid reusing it
...
def filter_peaks(residuals, idx_dist, idx_time, longi_offset, dx, gap_tresh = 15):
    ...
    mask_resi = abs(residuals) <  3 * rms_total# np.copy(mask_dist)    #abs(residuals) <  3 * rms_total #1.5 or 3 makes a difference
